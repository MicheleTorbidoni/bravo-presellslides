...
class PresaleRecapMailerTest < ActionMailer::TestCase
  test "recap builds with recipient, default sender, subject and the given body" do
    session = PresaleSession.new(company_name: "Acme Spa")
    mail = PresaleRecapMailer.recap(session, to: "prospect@acme.it", body: "Corpo del recap")
  ...
  test "the sender is configurable via RECAP_MAIL_FROM" do
    ...
    mail = PresaleRecapMailer.recap(session, to: "prospect@acme.it", body: "x")
