...
- **Tre superfici uniformi**: hub, flusso slide e pagina di chiusura condividono ora la **stessa cornice 16:9**, così la presentazione vista dal prospect è visivamente coerente dall'inizio alla fine.
- **Tutte e 13 le criticità navigabili**: ogni criticità ha contenuti placeholder (2–3 slide ciascuna, con i tre tipi) per poter scorrere avanti/indietro qualsiasi flusso.
...
- `app/frontend/components/present/Stage.tsx` (**nuovo**) — cornice "palco" **16:9 condivisa** (barre nere esterne + stage `aspect-[16/9] max-h-screen max-w-[177.78vh]` slate). Props `children` + `onClick?` (cursor-pointer solo quando passato). **Avvolge tutte e tre le superfici prospect** (hub, player, chiusura), così sono visivamente uniformi. È il punto unico dove M5 stilizzerà sfondo/letterbox/cornice.
- `app/frontend/components/present/SlidePlayer.tsx` (**nuovo**) — player **presentazionale** che usa `Stage` (`onClick` = avanti). Rende i tre tipi `concept` / `screenshot` / `sequence`; logo "Bravo Manufacturing" in angolo (collocazione grezza, rifinita in M5). Esporta `interpolate()` (sostituzione `{{company_name}}`/`{{contact_name}}` con fallback gentile) e il tipo `Slide`. `SlideImage` interno con `onError` → placeholder grigio etichettato col nome file. Le sequence mostrano dei pallini di progresso step.
- `app/frontend/components/present/Hub.tsx` e `Closing.tsx` (**modificati**) — avvolti in `<Stage>` (prima erano `min-h-screen` full-bleed): ora condividono il letterbox 16:9 del player. Comportamento invariato (rifinitura M3).
...
- **Criticità senza slide**: il player mostra una slide-placeholder navigabile ("Contenuto non ancora disponibile"); premere avanti conclude il flusso. (Nota: dopo il build, `slides.json` è stato popolato per **tutte e 13** le criticità con 2–3 slide placeholder ciascuna, quindi oggi il caso "senza slide" non si verifica in pratica — ma il fallback resta a protezione.)
...
- **Punti di styling**: tutta la veste prospect è in `app/frontend/components/present/` (`Stage.tsx`, `SlidePlayer.tsx`, `QuestionCapture.tsx`, `Hub.tsx`, `Closing.tsx`) — componenti autonomi, **niente design system del template**. M5 applica token/primitive Figma qui **senza cambiare il comportamento**. **Partire da `Stage.tsx`**: è la cornice 16:9 condivisa da hub/player/chiusura (sfondo, barre, dimensioni del palco), quindi stilizzarlo una volta allinea tutte e tre le superfici.
