...
Stai entrando in plan mode per pianificare e poi costruire il milestone 3 di questo progetto.

## Contesto

- Leggi `@_build_plan/prd.md` per il contesto completo del progetto: scope, modello dati e stack tecnologico.
- Leggi le cartelle dei milestone precedenti (`@_build_plan/milestones/1-*/milestone-log.md`, `@_build_plan/milestones/2-*/milestone-log.md`) per capire cosa è già stato costruito.
- **Nota UI critica:** da questo milestone inizia il contesto UI custom mostrato al prospect. Questi componenti React sono autonomi e **non** importano nulla dal design system del template.

## Il tuo compito

1. Pianifica l'implementazione **solo** del milestone 3 come definito nel PRD. Non pianificare né costruire nulla dei milestone successivi.
2. Dopo che l'utente ha confermato il piano, costruisci solo ciò che rientra nello scope del milestone 3.
3. Verifica il tuo lavoro rispetto ai criteri "Fatto quando" del milestone 3 nel PRD.
4. Al completamento, scrivi un `milestone-log.md` in questa cartella (`_build_plan/milestones/3-hub-criticita/milestone-log.md`). Strutturalo così:
